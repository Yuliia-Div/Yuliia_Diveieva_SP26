...
view.*/
CREATE OR REPLACE FUNCTION public.get_sales_revenue_by_category_qtr (qua_year DATE)
...
BEGIN
	...
	SELECT cr.name,
		SUM(CASE 
	        WHEN EXTRACT(QUARTER FROM payment_date) = EXTRACT(QUARTER FROM qua_year)
	        	AND EXTRACT(YEAR FROM payment_date) = EXTRACT(YEAR FROM qua_year)
	        THEN amount END) AS current_qtr_revenue,
	    SUM(CASE 
	        WHEN EXTRACT(YEAR FROM payment_date) = EXTRACT(YEAR FROM qua_year)
	        THEN amount 
	    	END) AS current_year_revenue
	FROM category_revenue cr
	GROUP BY cr.name
	HAVING SUM(CASE WHEN EXTRACT(YEAR FROM payment_date) = EXTRACT(YEAR FROM qua_year) THEN amount END) > 0;
END;
$$

--Test queries
SELECT *
FROM public.get_sales_revenue_by_category_qtr('2017-02-01'); --correct input

SELECT *
FROM public.get_sales_revenue_by_category_qtr('2017-12-01'); --uncorrect quarter

SELECT *
FROM public.get_sales_revenue_by_category_qtr(2-2017); --uncorrect format

SELECT *
FROM public.get_sales_revenue_by_category_qtr('2018-12-01'); --no data
...
AS $$
DECLARE
    clean_countries VARCHAR[]; -- added to remove whitespace and make it case-insensitive
BEGIN
	-- handles NULL input
    IF countries IS NULL THEN
        RAISE WARNING 'Input array is NULL. Returning empty result.';
        RETURN;
    END IF;

    -- handle empty array
    IF array_length(countries, 1) IS NULL THEN
        RAISE NOTICE 'Input array is empty. No countries to process.';
        RETURN;
    END IF;

    -- normalizes input
    SELECT ARRAY_AGG(LOWER(TRIM(n))) INTO clean_countries 
    FROM unnest(countries) AS n 
    WHERE n IS NOT NULL AND n <> '';

    -- Checks if any of the provided countries exist in our DB
    IF NOT EXISTS (
        SELECT 1 FROM public.country 
        WHERE LOWER(country) = ANY(clean_countries)
    ) THEN
        RAISE NOTICE 'None of the countries were found in the database';
        RETURN;
    END IF;
    ...
    WITH film_popularity AS (
        ...
        WHERE LOWER(co.country) = ANY (clean_countries)
...
--Test queries
SELECT *
FROM public.most_popular_films_by_countries(ARRAY['Canada', 'japan']);--correct

SELECT *
FROM public.most_popular_films_by_countries(ARRAY['Australia']);--incorrect country

SELECT *
FROM public.most_popular_films_by_countries(); --NULL input

SELECT *
FROM public.most_popular_films_by_countries(ARRAY[]::VARCHAR[]); -- empty array with type

SELECT *
FROM public.most_popular_films_by_countries(ARRAY[]);-- empty array without type

/*The most popular film is defined by the number of rentals in the countries based on customers addresses.
...
BEGIN
	...
	IF word IS NULL OR TRIM(word) = '' THEN
	    RAISE NOTICE 'Input string is empty or blank';
	    RETURN;
	END IF;
    ...
    WITH latest_rentals AS (
		...
		WHERE LOWER(f.title) LIKE ('%' || LOWER(TRIM(word)) || '%')
...
SELECT *
FROM public.films_in_stock_by_title(' love  ');--correct with spaces

SELECT *
FROM public.films_in_stock_by_title('something else');--no such films

SELECT *
FROM public.films_in_stock_by_title();--null input

SELECT *
FROM public.films_in_stock_by_title('');--empty input
...
BEGIN
	...
	IF x_release_year < 1895 OR x_release_year > (EXTRACT(YEAR FROM CURRENT_DATE) + 10) THEN
    RAISE EXCEPTION 'The year % is out of valid range (1895 - %).', 
                    x_release_year, 
                    (EXTRACT(YEAR FROM CURRENT_DATE) + 10);
	END IF;
	/*Postgres automatically tries to convert data in the needed format, so if it`s in the correct range it'll be 
	accepted but if it can't be converted then the function will raise Incorrect syntax Error*/
...
SELECT public.new_movie('Interstellar Journey2');--using default
...
SELECT public.new_movie('The first film', 1700); --inserting film with too early release year

SELECT public.new_movie('The last film', 3500); --inserting film with too late release year

SELECT public.new_movie('Any film', '2025'); --inserting release year in a string format

SELECT public.new_movie('Any film', '202'); --inserting release year in a string format but wrong release range

SELECT public.new_movie('Any film', 'two'); --inserting inconvertible release year
